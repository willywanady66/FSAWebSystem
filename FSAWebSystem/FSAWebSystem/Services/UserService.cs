...
namespace FSAWebSystem.Services
{
    public class UserService : IUserService
    {
        ...
        {
            var userUnilever = await _db.UsersUnilever.Include(x => x.BannerPlants.Where(x => x.IsActive)).ThenInclude(x => x.Banner).Include(x => x.RoleUnilever.Menus).Include(x => x.SKUs).Include(x => x.ProductCategories).SingleOrDefaultAsync(x => x.Id == id);
            return userUnilever;
        }
